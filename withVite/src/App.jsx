...
function App() {
	...
	const jobFilterByFilters = jobsData.filter((job) => {
		...
		const matchText = textToFilter === "" || job.title.toLowerCase().includes(textToFilter?.toLowerCase()); // significa si no hay nada escrito, la condición es true y no filtra nada y se muestra todo O si lo que escribe el usuario en minuscula hace match (está incluido) en el título que ha puesto la persona en el json hará match
		const matchTech = filters.technology === "" || job.data.technology.toLowerCase() === filters.technology.toLowerCase(); // si no filtra por nada, se muestra todo O si filta y el filtro en lower case hace match con la tecnología del json habvrá hecho match y así tanto para el location como para el experience
		const matchLocation = filters.location === "" || job.data.location.toLowerCase() === filters.location.toLowerCase();
		const matchExperience = filters.experience === "" || job.data.level.toLowerCase() === filters.experience.toLowerCase();
	...
	const jobFilterByTextFilter = textToFilter === "" 
		? jobFilterByFilters // si el texto es vacio, muestra todos los resultados o lo que haya en los filters
		: jobFilterByFilters.filter(job => job.title.toLowerCase().includes(textToFilter?.toLowerCase())); // si no, filtra por el texto y a través del título hace match con el texto que se escribe
	
	const totalPages = Math.ceil(jobFilterByTextFilter.length / MAX_RESULT_PER_PAGE); // como estoy filtrando los resultados, el total de páginas se va a ir cambiando y por ende, la paginación y esta dependerá de la cantidad de resultados que tenga y el MAX_RESULT_PER_PAGE
...
1. unificar o todo con submit o todo con onChange :check
	-> voy a hacerlo todo con submit porque parece más lógico aunque tenga que hacer un click adicional
	1.1 hacer que los filtros funcionen todos con el submit sea uno o todos a la vez
	1.2 hacer que el text to filter funcione con el submit y no con onChange
		-> lo he conseguido con ayuda de la AI porque estaba aparenciendo el event pero no quería hardcodearlos a través de event.target[0].value porque podría cambiar así que le he pedido que me ayude a hacerlo de otra manera.
		la solución ha sido a través del useId y recupero del formData el search
